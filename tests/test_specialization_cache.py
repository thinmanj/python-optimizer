...
from python_optimizer.specialization_cache import (
    SpecializationCache, CacheConfiguration, SpecializationEntry,
    TypeHasher, AdaptiveEvictionStrategy, EvictionPolicy,
    SpecializationMetrics, SpecializationType,
    get_global_cache, configure_cache, clear_cache, get_cache_stats
)
...
class TestSpecializationEntry:
    ...
    def test_memory_estimation(self):
        ...
        entry = SpecializationEntry("test_key", dummy_func, (1, 2, 3), {"key": "value"}, config)
...
class TestConcurrency:
    ...
    def test_concurrent_eviction(self):
        ...
        stats = cache.get_stats()
        assert stats['total_entries'] <= config.max_size * 1.2  # Allow some overshoot during concurrent operations
        assert stats['evictions'] > 0
...
class TestEdgeCases:
    ...
    def test_memory_pressure_simulation(self):
        ...
        assert stats['memory_usage_estimate'] <= config.max_memory_mb * 2  # Allow some overshoot

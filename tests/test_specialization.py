...
from python_optimizer import optimize, get_specialization_stats, clear_specialization_cache
from python_optimizer.specialization.analyzer import TypeAnalyzer, VariableUsage, TypePattern
from python_optimizer.specialization.cache import SpecializationCache, CacheEntry
from python_optimizer.specialization.dispatcher import RuntimeDispatcher, DispatchResult
from python_optimizer.specialization.engine import SpecializationEngine, SpecializationConfig
...
class TestTypeAnalyzer:
    ...
    def test_dominant_type_detection(self):
        ...
        assert dominant in {int, float}  # Should prefer numeric types for numeric operations
    
    def test_type_pattern_creation(self):
        """Test TypePattern creation and scoring."""
        pattern = TypePattern(
            function_name="test_func",
            parameter_types={"x": int, "y": float}
        )

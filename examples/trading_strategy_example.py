...
def calculate_technical_indicators(prices: np.ndarray, volume: np.ndarray = None) -> Dict:
    """
...
def backtest_strategy(prices: np.ndarray, 
                     indicators: Dict,
    ...
    for i in range(1, n):
        ...
        if position == 0:  # No position
            ...
            if (sma_20[i] > sma_50[i] and 
                sma_20[i-1] <= sma_50[i-1] and 
                rsi[i] < rsi_oversold and
                current_price < bb_lower[i] * 1.02):
            ...
            # SMA crossover down
            elif position > 0 and sma_20[i] < sma_50[i] and sma_20[i-1] >= sma_50[i-1]:
                should_exit = True
            ...
            if should_exit:
                ...
                profit = cash - initial_capital if len(trades) == 1 else cash - trades[-1]['price'] * shares
    ...
    win_rate = len(profitable_trades) / len([t for t in trades if 'profit' in t]) if trades else 0
...
def optimize_strategy_parameters(prices: np.ndarray,
                               indicators: Dict,
                               param_ranges: Dict) -> Tuple[Dict, List]:
    """
    ...
    total_combinations = (len(sma_short_range) * len(sma_long_range) * 
                         len(rsi_oversold_range) * len(rsi_overbought_range))
            ...
            for rsi_oversold in rsi_oversold_range:
                for rsi_overbought in rsi_overbought_range:
                    ...
                    if combination_count % 50 == 0:
                        print(f"  Tested {combination_count} combinations... "
                              f"Best Sharpe: {best_sharpe:.3f}")
    
    return best_params, all_results


def generate_market_data(n_days: int = 500) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def visualize_results(prices: np.ndarray, 
    ...
    ax1.fill_between(dates, indicators['bb_lower'], indicators['bb_upper'], 
                     alpha=0.2, label='Bollinger Bands')
    
    # Mark trades
    for trade in trades:
        if trade['type'] == 'BUY':
            ax1.scatter(dates[trade['day']], trade['price'], color='green', marker='^', s=100, alpha=0.7)
        else:
            ax1.scatter(dates[trade['day']], trade['price'], color='red', marker='v', s=100, alpha=0.7)
    ...
    # Save the plot
    output_path = '/Users/julio/Projects/python-optimizer/examples/trading_strategy_analysis.png'
...
def main():
    ...
    print("=" * 60)
    print("Demonstrating high-performance trading strategy backtesting with Python Optimizer")
    print()
    ...
    start_time = time.perf_counter()
    best_params, all_results = optimize_strategy_parameters(prices, indicators, param_ranges)
    ...
    visualize_results(prices, best_result['portfolio_values'], indicators, 
                     best_result['trades'], dates)
    
    print(f"\n✅ Trading strategy optimization completed successfully!")
    print(f"🚀 Achieved {throughput:.1f} backtests per second with specialized optimization")

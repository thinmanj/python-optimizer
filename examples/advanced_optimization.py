...
def example_1_variable_specialization():
    ...
    for data, data_type in test_cases:
        ...
        for operation in operations:
            ...
            result_str = f"{result:.3f}" if isinstance(result, (int, float)) else str(result)[:12]
...
def example_2_adaptive_learning():
    ...
    def adaptive_algorithm(data, method='fast'):
        ...
        if method == 'fast':
            ...
            if hasattr(data, '__len__'):
                return adaptive_algorithm(data, 'accurate' if len(data) > 1000 else 'fast')
            else:
    ...
    for i in range(20):
        ...
        if i % 5 == 0:
            ...
            print(f"  Step {i+1:2d}: {stats.get('specializations_created', 0)} specs, "
    ...
    for i in range(20):
        ...
        if i % 5 == 0:
            ...
            print(f"  Step {i+1:2d}: Memory: {cache_stats['memory_usage_estimate']:.1f}MB, "
                  f"hit rate: {stats.get('cache_hit_rate', 0):.1%}")
...
def example_3_financial_computing():
    ...
    def advanced_portfolio_metrics(returns, weights, benchmark_returns=None, 
                                 risk_free_rate=0.02):
        """
    ...
    for strategy_name, weights in strategies.items():
        ...
        for _ in range(50):
            start = time.perf_counter()
            metrics = advanced_portfolio_metrics(returns_list, weights_list, benchmark_list)
            times_list.append(time.perf_counter() - start)
        
        # Store results
        results[strategy_name] = metrics
        performance_times[strategy_name] = {
            'array': times_array,
            'list': times_list
        }
...
def example_4_machine_learning():
    ...
    def optimized_kmeans_step(data, centroids, max_iter=100):
        ...
        for iteration in range(max_iter):
            ...
            for j in range(k):
                if counts[j] > 0:
                    for d in range(new_centroids.shape[1] if new_centroids.ndim > 1 else 1):
...
def example_5_cache_management():
    ...
    print(f"Specializations created: {final_func_stats.get('specializations_created', 0)}")
...
def create_performance_visualization(performance_data):
    ...
    bars1 = ax1.bar(data_types, [t * 1000 for t in sum_times], alpha=0.7, color='skyblue')
    ...
    for bar, time_val in zip(bars1, sum_times):
        ...
        ax1.text(bar.get_x() + bar.get_width()/2., height + 0.001,
                f'{time_val*1000:.3f}ms', ha='center', va='bottom', fontsize=8)
    ...
    ax3.fill_between(iterations, memory_usage, alpha=0.6, color='orange', label='Memory Usage')
    ...
    scenarios = ['Fibonacci', 'Statistics', 'Financial', 'ML K-means', 'Data Processing']
    speedups = [51, 25, 85, 120, 45]  # Example speedup factors
    
    bars4 = ax4.bar(scenarios, speedups, alpha=0.7, 
...
def main():
    ...
    try:
        ...
        # Example 3: Financial computing
        results['financial'], results['financial_perf'] = example_3_financial_computing()
        
        # Example 4: Machine learning
        results['ml'] = example_4_machine_learning()
        
        # Example 5: Cache management
        results['cache_evolution'], results['cache_final'] = example_5_cache_management()

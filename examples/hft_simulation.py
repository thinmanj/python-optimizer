...
def process_order_book_update(prices, quantities, sides, new_price, new_quantity, new_side):
...
@optimize(jit=True, fastmath=True, nogil=True)
def process_market_data_tick(prices_buffer, volumes_buffer, timestamps_buffer, 
                            new_price, new_volume, new_timestamp, buffer_size):
...
@optimize(jit=True, fastmath=True, nogil=True)
def fast_order_matching(buy_prices, buy_quantities, sell_prices, sell_quantities, max_levels):
...
def benchmark_hft_system():
    ...
    for _ in range(10000):
        exposure, var, breach = calculate_portfolio_risk(positions, prices, limits, volatilities)
    ...
    for _ in range(10000):
        volume, value = fast_order_matching(
            buy_prices.copy(), buy_quantities.copy(),
            sell_prices.copy(), sell_quantities.copy(), 5
        )
    ...
    print(f"Positions:             {len([p for p in strategy.positions.values() if p != 0])} active")
    
    # Summary
    print("\n🏆 HFT System Performance Summary")
    print("=" * 60)
    print(f"Market Data Throughput:    {processed_count/processing_time:.0f} ticks/second")

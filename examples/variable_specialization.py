...
from python_optimizer import optimize, get_specialization_stats, configure_specialization
...
def calculate_fibonacci_specialized(n):
    ...
    if n <= 1:
        return n
    return calculate_fibonacci_specialized(n-1) + calculate_fibonacci_specialized(n-2)
...
def demo_numeric_specialization():
    ...
    python_results = benchmark_function(calculate_fibonacci_python, test_cases, iterations=100)
    spec_results = benchmark_function(calculate_fibonacci_specialized, test_cases, iterations=100)
    full_results = benchmark_function(calculate_fibonacci_full, test_cases, iterations=100)
    
    print("Results:")
    print(f"Pure Python:         {python_results['mean_time']:.4f}s ± {python_results['std_time']:.4f}s")
    print(f"Specialization only: {spec_results['mean_time']:.4f}s ± {spec_results['std_time']:.4f}s")
    print(f"JIT + Specialization: {full_results['mean_time']:.4f}s ± {full_results['std_time']:.4f}s")
...
def demo_array_specialization():
    ...
    python_list_results = benchmark_function(matrix_multiply_python, list_test_cases, iterations=20)
    spec_list_results = benchmark_function(matrix_multiply_specialized, list_test_cases, iterations=20)
    
    # Benchmark with NumPy arrays
    numpy_test_cases = [(A_numpy, B_numpy)]
    spec_numpy_results = benchmark_function(matrix_multiply_specialized, numpy_test_cases, iterations=100)
    
    # Direct NumPy comparison
    def numpy_direct(A, B):
        return A @ B
    
    numpy_direct_results = benchmark_function(numpy_direct, numpy_test_cases, iterations=100)
...
def demo_container_specialization():
    ...
    for container_name, container_data in [("list", list_data), ("tuple", tuple_data), ("array", array_data)]:
        test_cases = [(container_data, op) for op in operations]
        
        python_results = benchmark_function(process_data_python, test_cases, iterations=50)
        spec_results = benchmark_function(process_data_specialized, test_cases, iterations=50)
...
def run_comprehensive_benchmark():
    ...
    print(f"  Specializations created:     {adaptive_stats.get('specializations_created', 0)}")
    print(f"  Total specialized calls:     {adaptive_stats.get('specialized_calls', 0)}")
    print(f"  Cache hit rate:              {adaptive_stats.get('cache_hit_rate', 0):.2%}")
    
    # Overall statistics
    global_stats = get_specialization_stats()
    print(f"\nGlobal Specialization Statistics:")
    print(f"  Functions optimized:         {global_stats.get('functions_optimized', 0)}")
    print(f"  Total calls processed:       {global_stats.get('total_calls', 0)}")
    print(f"  Global specialization rate:  {global_stats.get('global_specialization_rate', 0):.2%}")
    print(f"  Global cache hit rate:       {global_stats.get('global_cache_hit_rate', 0):.2%}")
...
if __name__ == "__main__":
    ...
    print(f"💡 Consider enabling specialization for production workloads with mixed-type inputs.")

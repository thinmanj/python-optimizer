...
def benchmark_ml_algorithms():
    ...
    X_gd, y_gd = make_classification(n_samples=5000, n_features=20, 
    ...
    print("\n🏆 Performance Summary")
    print("=" * 60)
    avg_speedup = np.mean([results['kmeans']['speedup'], results['neural_network']['speedup']])
    print(f"Average ML Algorithm Speedup: {avg_speedup:.2f}x")
    print(f"K-Means Clustering:          {results['kmeans']['speedup']:.2f}x faster")
    print(f"Neural Network Forward:      {results['neural_network']['speedup']:.2f}x faster")

...
class SpecializationCache:
    ...
    def __init__(self, max_size: int = 1000, enable_persistence: bool = True):
        ...
        self._function_keys: Dict[str, Set[str]] = defaultdict(set)  # func_name -> cache_keys
    ...
    def get(self, func_name: str, param_types: Tuple[type, ...], 
            usage_patterns: Dict[str, Any]) -> Optional[CacheEntry]:
        """Get a cached specialized version if available."""
        with self._lock:
            cache_key = self._generate_lookup_key(func_name, param_types, usage_patterns)
    ...
    def _generate_lookup_key(self, func_name: str, param_types: Tuple[type, ...], 
    ...
    def _evict_entries(self, num_to_evict: Optional[int] = None):
        ...
        for func_name, entries in self._cache.items():
            for cache_key, entry in entries.items():
                all_entries.append((func_name, cache_key, entry, entry.get_cache_score()))
    ...
    def get_function_stats(self, func_name: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            avg_gain = sum(entry.performance_gain for entry in entries.values()) / len(entries)

...
from .cache import SpecializationCache, CacheEntry, get_global_cache
...
class RuntimeDispatcher:
    ...
    def dispatch(self, func: types.FunctionType, args: Tuple, kwargs: Dict[str, Any]) -> DispatchResult:
        ...
        if should_specialize:
            specialized_func = self._create_specialization(func, param_types, usage_patterns)
            if specialized_func:
    ...
    def _analyze_argument_types(self, func: types.FunctionType, 
                              args: Tuple, kwargs: Dict[str, Any]) -> Dict[str, type]:
    ...
    def _get_usage_patterns(self, func_name: str, param_types: Dict[str, type]) -> Dict[str, Any]:
    ...
    def _should_specialize(self, func_name: str, param_types: Dict[str, type]) -> Tuple[bool, str]:
        ...
        if func_stats.get('specializations', 0) >= self.max_specializations_per_function:
    ...
    def _create_specialization(self, func: types.FunctionType, 
                             param_types: Dict[str, type], 
                             usage_patterns: Dict[str, Any]) -> Optional[Callable]:
    ...
    def get_dispatch_stats(self, func_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        with self._lock:
            if func_name:
                ...
                # Global stats
                total_dispatches = sum(len(times) for times in self.dispatch_stats.values())
                avg_dispatch_time = 0.0
                if total_dispatches > 0:
                    all_times = [time for times in self.dispatch_stats.values() for time in times]
    ...
    def configure(self, min_calls: Optional[int] = None,
...
class AdaptiveDispatcher(RuntimeDispatcher):
    ...
    def record_performance(self, func_name: str, specialization_key: str, 
                          execution_time: float, is_specialized: bool):
        """Record actual performance for adaptive learning."""
        key = f"{func_name}:{specialization_key}" if is_specialized else f"{func_name}:original"
        self.performance_history[key].append(execution_time)
        
        # Keep only recent history
        if len(self.performance_history[key]) > self.adaptation_window:
            self.performance_history[key] = self.performance_history[key][-self.adaptation_window:]
    ...
    def _should_specialize(self, func_name: str, param_types: Dict[str, type]) -> Tuple[bool, str]:
        ...
        positive_feedback = any(
            eff > 0.2 for key, eff in self.specialization_effectiveness.items()
    ...
    def get_effectiveness_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'performance_history_size': {k: len(v) for k, v in self.performance_history.items()},
...
def dispatch_function_call(func: types.FunctionType, args: Tuple, 
                         kwargs: Dict[str, Any]) -> DispatchResult:

...
class SpecializationGenerator(ABC):
    ...
    @abstractmethod
    def generate_specialized_code(self, original_func: types.FunctionType, 
                                param_name: str, param_type: type, 
                                usage_pattern: Dict[str, Any]) -> str:
        """Generate specialized code for the given parameters."""
        pass
    
    @abstractmethod
    def estimate_performance_gain(self, param_type: type, usage_pattern: Dict[str, Any]) -> float:
...
class NumericSpecializer(SpecializationGenerator):
    ...
    def generate_specialized_code(self, original_func: types.FunctionType, 
        ...
        try:
            ...
            optimized_body = self._optimize_numeric_operations(body_lines, param_name, param_type)
...
"""
    ...
    def _optimize_numeric_operations(self, body_lines: List[str], param_name: str, param_type: type) -> str:
        ...
        for line in body_lines:
            ...
            if param_type == int:
                ...
                optimized_line = optimized_line.replace(f'{param_name} / ', f'{param_name} // ')  # Use floor division
            ...
            if f'{param_name} **' in optimized_line:
                ...
                if f'{param_name} ** 2' in optimized_line:
                    optimized_line = optimized_line.replace(f'{param_name} ** 2', f'{param_name} * {param_name}')
                elif f'{param_name} ** 0.5' in optimized_line:
                    optimized_line = optimized_line.replace(f'{param_name} ** 0.5', f'np.sqrt({param_name})')
            
            optimized_lines.append(optimized_line)
        
        return ''.join(optimized_lines)
    
    def _generate_simple_wrapper(self, original_func: types.FunctionType, param_name: str, param_type: type) -> str:
...
class ArraySpecializer(SpecializationGenerator):
    ...
    def can_specialize(self, param_type: type, usage_pattern: Dict[str, Any]) -> bool:
        ...
        return (param_type == np.ndarray or 
                usage_pattern.get('is_array_indexed', False) or
                param_type in {list, tuple} and usage_pattern.get('is_numeric_heavy', False))
...
"""
            ...
            return self._generate_simple_array_wrapper(original_func, param_name, param_type)
    
    def _generate_simple_array_wrapper(self, original_func: types.FunctionType, param_name: str, param_type: type) -> str:
...
class ContainerSpecializer(SpecializationGenerator):
    ...
    def can_specialize(self, param_type: type, usage_pattern: Dict[str, Any]) -> bool:
        ...
        return param_type in {list, dict, tuple, set} or usage_pattern.get('is_container', False)
...
class SpecializationCodeGenerator:
    ...
    def generate_specialized_function(self, original_func: types.FunctionType,
                                    param_name: str, param_type: type,
                                    usage_pattern: Dict[str, Any]) -> Optional[Callable]:
        ...
        try:
            ...
            for name, obj in namespace.items():
                if callable(obj) and name.startswith(f'{original_func.__name__}_specialized'):
    ...
    def estimate_total_gain(self, original_func: types.FunctionType,
                          type_patterns: Dict[str, type]) -> float:
        ...
        for param_name, param_type in type_patterns.items():
            ...
            for generator in self.generators:
                if generator.can_specialize(param_type, usage_pattern):
                    gain = generator.estimate_performance_gain(param_type, usage_pattern)
                    total_gain = max(total_gain, gain)  # Take best single specialization
                    break

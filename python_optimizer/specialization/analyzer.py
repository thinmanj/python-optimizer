...
class VariableUsage:
    ...
    def add_operation(self, op: str):
        ...
        container_ops = {'append', 'extend', 'pop', 'remove', 'insert', '__getitem__', '__setitem__'}
...
class TypePattern:
    ...
    def _calculate_potential(self) -> float:
        ...
        for param_type in self.parameter_types.values():
            if param_type in numeric_types or (hasattr(param_type, '__module__') and 'numpy' in str(param_type.__module__)):
                score += 0.3
...
class TypeAnalyzer:
    ...
    def __init__(self):
        self.runtime_types: Dict[str, Dict[str, List[type]]] = defaultdict(lambda: defaultdict(list))
    ...
    def _enhance_with_type_hints(self, func: types.FunctionType, variables: Dict[str, VariableUsage]):
    ...
    def analyze_call_patterns(self, func_name: str, min_calls: int = 5) -> List[TypePattern]:
        ...
        for param_name, type_list in call_data.items():
            ...
            dominant_types = {t: count for t, count in type_counts.items() 
                            if count >= min_calls * 0.3}  # At least 30% of calls
        ...
        if type_combinations:
            ...
            for param_name, type_counts in param_groups.items():
                ...
                pattern = TypePattern(
                    ...
                    parameter_types={param_name: dominant_type}
                )
    ...
    def get_specialization_candidates(self, func_name: str) -> List[TypePattern]:
        ...
        candidates = sorted(patterns, key=lambda p: p.optimization_potential, reverse=True)
    ...
    def get_analysis_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'specialization_patterns': sum(len(patterns) for patterns in self.call_patterns.values()),

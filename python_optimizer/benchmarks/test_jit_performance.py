...
def generate_synthetic_market_data(days: int = 1000, initial_price: float = 100.0) -> pd.DataFrame:
...
def test_individual_jit_functions():
    ...
    logger.info(f"Returns calculation: {returns_time*1000:.2f}ms, result shape: {returns.shape}")
    
    # Test Sharpe ratio
    start_time = time.perf_counter()
    sharpe = calculate_sharpe_ratio_jit(returns)
    sharpe_time = time.perf_counter() - start_time
    logger.info(f"Sharpe ratio calculation: {sharpe_time*1000:.2f}ms, result: {sharpe:.4f}")
    ...
    logger.info(f"Max drawdown calculation: {drawdown_time*1000:.2f}ms, result: {max_dd:.4f}")
    
    # Test signal generation
    start_time = time.perf_counter()
    ma_signals = generate_ma_signals_jit(prices, 3, 5)
    ma_time = time.perf_counter() - start_time
    logger.info(f"MA signal generation: {ma_time*1000:.2f}ms, signals: {np.sum(np.abs(ma_signals))}")
    
    start_time = time.perf_counter()
    rsi_signals = generate_rsi_signals_jit(prices, 5, 30, 70)
    rsi_time = time.perf_counter() - start_time
    logger.info(f"RSI signal generation: {rsi_time*1000:.2f}ms, signals: {np.sum(np.abs(rsi_signals))}")
...
def benchmark_jit_evaluator():
    ...
    for data_size in data_sizes:
        for pop_size in population_sizes:
            ...
            jit_evaluator = JITBacktestFitnessEvaluator(initial_cash=10000, commission=0.001)
...
def generate_summary_report(results: List[Dict]):
    ...
    # Detailed breakdown
    logger.info(f"\\nDetailed Results:")
    logger.info(f"{'Data Size':<10} {'Pop Size':<10} {'Time (s)':<10} {'Eval/s':<10} {'Success %':<12} {'Avg Fitness':<12}")
    logger.info("-" * 70)
    
    for result in results:
        logger.info(f"{result['data_size']:<10} {result['population_size']:<10} "
    ...
    logger.info(f"\\nPERFORMANCE INSIGHTS:")
    logger.info(f"• JIT compilation provides significant speedup for numerical computations")

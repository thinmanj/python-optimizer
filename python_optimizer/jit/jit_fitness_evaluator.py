...
from ..genetic.genetic_optimizer import Individual, FitnessEvaluator
...
@njit(cache=True, fastmath=True) if NUMBA_AVAILABLE else lambda f: f
def calculate_sharpe_ratio_jit(returns: np.ndarray, risk_free_rate: float = 0.0) -> float:
...
def simulate_strategy_jit(signals: np.ndarray, 
                         prices: np.ndarray, 
                         initial_cash: float,
...
@njit(cache=True, fastmath=True) if NUMBA_AVAILABLE else lambda f: f
def generate_ma_signals_jit(prices: np.ndarray, short_window: int, long_window: int) -> np.ndarray:
...
@njit(cache=True, fastmath=True) if NUMBA_AVAILABLE else lambda f: f
def generate_rsi_signals_jit(prices: np.ndarray, rsi_period: int, oversold: float, overbought: float) -> np.ndarray:
...
class JITBacktestFitnessEvaluator(FitnessEvaluator):
    ...
    def __init__(self, strategy_class: Optional[type] = None, initial_cash: float = 10000, commission: float = 0.001):
    ...
    def _prepare_data(self, data: pd.DataFrame) -> Dict[str, np.ndarray]:
        ...
        result = {
            'close': data['close'].values if 'close' in data.columns else data.iloc[:, -1].values,
    ...
    def evaluate(self, individual: Individual, data: pd.DataFrame) -> Dict[str, float]:
        ...
        try:
            ...
            metrics = self._calculate_jit_metrics(equity_curve, trade_returns, total_trades)
    ...
    def _generate_signals(self, genes: Dict[str, Any], data: Dict[str, np.ndarray]) -> np.ndarray:
    ...
    def _calculate_jit_metrics(self, equity_curve: np.ndarray, 
                              trade_returns: np.ndarray, total_trades: int) -> Dict[str, float]:

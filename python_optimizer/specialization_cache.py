...
class SpecializationMetrics:
    ...
    def update_access(self, execution_time: float = 0.0, success: bool = True):
        ...
        if execution_time > 0:
            ...
            self.average_execution_time = self.total_execution_time / max(1, self.hit_count)
        ...
        try:
            ...
            with cls._lock:
                if value_type not in cls._type_cache:
                    cls._type_cache[value_type] = cls._generate_type_signature(value_type)
...
class SpecializationEntry:
    ...
    def __init__(self, key: str, specialized_func: Callable, 
                 original_args: Tuple[Any, ...], original_kwargs: Dict[str, Any],
    ...
    def is_valid(self) -> bool:
        ...
        if (self.config.ttl_seconds and 
            time.time() - self.metrics.creation_time > self.config.ttl_seconds):
            return False
...
class AdaptiveEvictionStrategy:
    ...
    def should_evict(self, entries: Dict[str, SpecializationEntry], 
                    current_memory_mb: float, current_size: int) -> List[str]:
        ...
        return self._select_eviction_candidates(
            entries, memory_to_free, items_to_evict
        )
    ...
    def _evaluate_policy_performance(self, entries: Dict[str, SpecializationEntry]):
        ...
        if current_score < 0.6 and len(self.policy_performance) > 1:
            ...
            if best_policy != self.current_policy:
                logger.info(f"Switching eviction policy from {self.current_policy} to {best_policy}")
    ...
    def _calculate_hit_rate(self, entries: Dict[str, SpecializationEntry]) -> float:
        ...
        total_attempts = sum(entry.metrics.hit_count + entry.metrics.miss_count 
                           for entry in entries.values())
        return total_hits / max(1, total_attempts)
    
    def _calculate_avg_access_time(self, entries: Dict[str, SpecializationEntry]) -> float:
    ...
    def _select_eviction_candidates(self, entries: Dict[str, SpecializationEntry],
                                  memory_to_free: float, items_to_evict: int) -> List[str]:
    ...
    def _lru_candidates(self, entries: Dict[str, SpecializationEntry], 
                       count: int) -> List[str]:
        """Select LRU candidates."""
        sorted_entries = sorted(
            entries.items(), 
            key=lambda x: x[1].metrics.last_access_time
        )
        return [key for key, _ in sorted_entries[:count]]
    
    def _lfu_candidates(self, entries: Dict[str, SpecializationEntry], 
                       count: int) -> List[str]:
        """Select LFU candidates."""
        sorted_entries = sorted(
            entries.items(),
            key=lambda x: x[1].metrics.access_count
        )
        return [key for key, _ in sorted_entries[:count]]
    
    def _size_based_candidates(self, entries: Dict[str, SpecializationEntry],
                             memory_to_free: float, items_to_evict: int) -> List[str]:
        ...
        for key, entry in sorted_entries:
            ...
            freed_memory += entry.metrics.memory_estimate / (1024 * 1024)  # Convert to MB
            
        return candidates
    
    def _adaptive_candidates(self, entries: Dict[str, SpecializationEntry],
                           memory_to_free: float, items_to_evict: int) -> List[str]:
        ...
        return [key for key, _ in scored_entries[:max(items_to_evict, len(scored_entries))]]
...
class SpecializationCache:
    ...
    def get_key(self, func_name: str, args: Tuple[Any, ...], 
    ...
    def put(self, key: str, specialized_func: Callable,
           original_args: Tuple[Any, ...], original_kwargs: Dict[str, Any]) -> SpecializationEntry:
        """Put entry into cache."""
        with self._lock:
            # Create new entry
            entry = SpecializationEntry(key, specialized_func, original_args, original_kwargs, self.config)
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            if total_lookups > 0:
                ...
                current_stats['miss_rate'] = current_stats['cache_misses'] / total_lookups
            else:
            ...
            current_stats['uptime_hours'] = (time.time() - current_stats['creation_time']) / 3600
            
            # Entry-level statistics
            if self._entries:
                access_counts = [entry.metrics.access_count for entry in self._entries.values()]
                current_stats['avg_access_count'] = sum(access_counts) / len(access_counts)
                current_stats['max_access_count'] = max(access_counts)
                current_stats['min_access_count'] = min(access_counts)
                
                ages = [time.time() - entry.metrics.creation_time for entry in self._entries.values()]
    ...
    def _maybe_evict(self):
        ...
        current_memory_mb = sum(entry.metrics.memory_estimate for entry in self._entries.values()) / (1024 * 1024)
        
        candidates = self._eviction_strategy.should_evict(self._entries, current_memory_mb, current_size)
    ...
    def _update_memory_stats(self):
        ...
        total_memory = sum(entry.metrics.memory_estimate for entry in self._entries.values())
    ...
    def maintenance(self):
        ...
        with self._lock:
            ...
            if invalid_keys:
                logger.debug(f"Removed {len(invalid_keys)} invalid entries during maintenance")
